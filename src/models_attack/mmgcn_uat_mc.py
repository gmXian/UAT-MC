...
class MMGCN_UAT_MC(GeneralRecommender):
    ...
    def generate_adversarial_noise(self, interaction):
        ...
        if attack_goal == "1":
            ...
            perturbed_feat_t = torch.clamp(perturbed_feat_t, 
                                            ...
                                            self.aligned_item_t_features + epsilon_t)
            bpr_loss = self.calculate_BPR_loss(interaction,self.aligned_item_v_features,perturbed_feat_t)
            bpr_loss.backward(retain_graph=False)
            with torch.no_grad():
            ...
            perturbed_feat_t = self.aligned_item_t_features + final_perturbation_t
    ...
    def calculate_loss(self, interaction):
        ...
        all_loss = loss + reg_loss + noisy_loss
        return all_loss
