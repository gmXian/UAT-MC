...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        # dataframe
        self.uid_field = self.config['USER_ID_FIELD'] # userID
        self.iid_field = self.config['ITEM_ID_FIELD'] # itemID
        self.splitting_label = self.config['inter_splitting_label'] # 'x_label'
    ...
    def split(self):
        ...
        for i in range(3):
            ...
            temp_df.drop(self.splitting_label, inplace=True, axis=1)        # 删除splitting_label列，因为该列在后续不再使用。
            dfs.append(temp_df)
        if self.config['filter_out_cod_start_users']: # True
        ...
        # wrap as RecDataset
        full_ds = [self.copy(_) for _ in dfs] # 使用列表推导式，将分割后的数据框dfs中的每个数据框封装为RecDataset对象
        return full_ds
