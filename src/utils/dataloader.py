...
class TrainDataLoader(AbstractDataLoader):
    ...
    """
    def __init__(self, config, dataset, batch_size=1, shuffle=False): # shuffle 是否在每个epoch开始时打乱数据。
        super().__init__(config, dataset, additional_dataset=None, # 训练Dataloader不需要additional_dataset
                         batch_size=batch_size, neg_sampling=True, shuffle=shuffle)

        # special for training dataloader
        self.history_items_per_u = dict() # 存储每个用户的历史交互物品。
        ...
        # if full sampling
        self.use_full_sampling = config['use_full_sampling'] # False
        ...
        self._get_history_items_u() # 初始化每个用户的历史交互物品
        self.neighborhood_loss_required = config['use_neighborhood_loss'] # 似乎为None，未定义
    ...
    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None):
        ...
        if value_field is None: # 如果未指定value_field，则矩阵中的值默认为   1  。
    ...
    def _get_neg_sample(self): # 负采样
        cur_data = self.dataset[self.pr: self.pr + self.step] # cur_data是当前批次的数据，从self.dataset中提取。
        self.pr += self.step
        # to tensor
        ...
        if self.neighborhood_loss_required: # 不会执行
    ...
    def _get_non_neg_sample(self): # 不进行负采样的
    ...
    def _sample_neg_ids(self, u_ids): # 为这个batch的用户分别采样一个负样本
        neg_ids = []
        for u in u_ids:
            # random 1 item
            iid = self._random()
    ...
    def _random(self): # 从所有的商品中采样出一个item作为负样本
        rd_id = random.sample(self.all_items, 1)[0]
        return rd_id

    def _get_history_items_u(self): # 似乎不包含测试集和验证集
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        if additional_dataset is None: # 评估数据集的额外的训练数据集是训练集
    ...
    @property
    def pr_end(self):
        return self.eval_u.shape[0] # 返回评估数据集中用户的总数。

    def _shuffle(self):
        self.dataset.shuffle() # 打乱数据方法

    def _next_batch_data(self): # 生成批次数据
