...
class Config(object):
    ...
    def __init__(self, model=None, dataset=None, config_dict=None, mg=False,attack=False,is_AT=False):
        """
        Args:
        ...
        # model type
        self.final_config_dict = self._load_dataset_model_config(config_dict, mg, attack,is_AT) 
        # config in cmd and main.py are latest
        self.final_config_dict.update(config_dict)
        self._set_default_parameters()
        self._init_device()

    def _load_dataset_model_config(self, config_dict, mg, attack,is_AT):
        file_config_dict = dict()
        file_list = []
        # get dataset and model files
        cur_dir = os.getcwd()
        if attack:
            cur_dir = os.path.join(cur_dir, 'configs_attack')
            ...
            file_list.append(os.path.join(cur_dir, "model", "{}_{}.yaml".format(config_dict['model'],config_dict['dataset'])))
        elif is_AT:
            cur_dir = os.path.join(cur_dir, 'configs_AT')  # 对抗训练的配置
            file_list.append(os.path.join(cur_dir, "overall.yaml"))
            file_list.append(os.path.join(cur_dir, "dataset", "{}.yaml".format(config_dict['dataset'])))
            file_list.append(os.path.join(cur_dir, "model", "{}_{}.yaml".format(config_dict['model'],config_dict['dataset'])))
        else:
