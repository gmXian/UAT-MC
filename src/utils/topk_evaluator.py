...
class TopKEvaluator(object):
    ...
    def __init__(self, config):
        self.config = config
        self.metrics = config['metrics'] # ["Recall", "NDCG", "Precision", "MAP"]
        self.topk = config['topk'] # [5, 10, 20, 50]
        self.save_recom_result = config['save_recommended_topk'] # True
        self._check_args() # 在检查上面几个参数的正确性
    ...
    def collect(self, interaction, scores_tensor, full=False):
        ...
        return topk_index # 返回每个用户的top-k列表
