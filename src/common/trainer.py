...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        if 'AT' in config['model']:
            self.checkpoint_dir = self.checkpoint_dir + '/{}_{}/{}/{}-{}/'.format(self.config['model'],self.config['dataset'],self.config['epsilons'], self.config['adv_weight'], self.config['adv_align_weight'])
        if not os.path.exists(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)
        
        now_time = get_local_time()
        saved_model_file = '{}_{}_{}.pth'.format(self.config['model'],self.config['dataset'], now_time)
        self.saved_model_file = os.path.join(self.checkpoint_dir, saved_model_file)
        self.writer = SummaryWriter(log_dir='./loss_logs/{}_{}_{}'.format(self.config['model'],self.config['dataset'],now_time))
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            losses,rec_loss, reg_loss, align_loss = loss_func(interaction)
            ...
            if isinstance(losses, tuple):
                ...
                total_reg_loss = reg_loss.item() if total_reg_loss is None else total_reg_loss + reg_loss.item()
                total_align_loss = align_loss.item() if total_align_loss is None else total_align_loss + align_loss.item()
            # self.logger.info(f"rec_loss: {rec_loss}, reg_loss: {reg_loss}, align_loss: {align_loss}")
            if self._check_nan(loss):
                self.logger.info('Loss is nan at epoch: {}, batch index: {}. Exiting.'.format(epoch_idx, batch_idx))
                self.logger.info(f"rec_loss: {rec_loss}, reg_loss: {reg_loss}, align_loss: {align_loss}")
                return loss, torch.tensor(0.0)
            
            if self.mg and batch_idx % self.beta == 0: # mirror gradient
        ...
        self.writer.add_scalar('train_loss', total_loss, epoch_idx)
        self.writer.add_scalar('rec_loss', total_rec_loss, epoch_idx)
        self.writer.add_scalar('reg_loss', total_reg_loss, epoch_idx)
        self.writer.add_scalar('align_loss', total_align_loss, epoch_idx)
    ...
    def _save_checkpoint(self, epoch,dir=None):
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if (epoch_idx + 1) % 10 == 0 and epoch_idx<50 and 'AT' in self.config['model']:
                    saved_model_file_epoch = '{}_{}_{}_{}.pth'.format(self.config['model'],self.config['dataset'], get_local_time(), epoch_idx+1)
                    saved_model_file_epoch = os.path.join(self.checkpoint_dir, saved_model_file_epoch)
                    self._save_checkpoint(epoch_idx, dir=saved_model_file_epoch)
                    update_output = '█ Model saved at: %s' % saved_model_file_epoch
                    if verbose:
                        self.logger.info(update_output)
